...
} from "@/lib/server/gps-beacon/history-throttle";
import {
  isRecord,
  isSchemaColumnError,
  normalizePassengerCount,
} from "@/lib/server/gps-beacon/request-utils";
import { handleStatusOnlyTelemetry } from "@/lib/server/gps-beacon/status-only";
 ...
 */
export async function handleGpsBeaconPost(request: NextRequest) {
  ...
  if (isStatusOnly) {
    return handleStatusOnlyTelemetry({
      supabase,
...
export function getGpsBeaconHealth() {
