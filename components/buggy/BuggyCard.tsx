...
type BuggyCardProps = {
  ...
  /** Index dalam daftar untuk staggered entrance animation. */
  index?: number;
  /** Apakah buggy ini di-bintang user. Hanya relevan saat `canFavorite`. */
  isFavorite?: boolean;
  /** Toggle favorit. Wajib disediakan saat `canFavorite=true`. */
...
export function BuggyCard({
  ...
  // Buggy image masuk sedikit setelah card muncul agar terasa berurutan.
  const driveDelayMs = cardDelayMs + 120;
  ...
  return (
    <article
      ...
      className={`buggy-card-enter group relative flex w-full flex-col justify-between overflow-hidden rounded-[20px] border border-slate-200/80 p-2.5 text-left transition-all hover:bg-slate-50 active:scale-[0.98] ${
        isSelected
    ...
    >
      {/* Background Vehicle Image — drives in dari kanan ke kiri */}
      <div
        style={{ animationDelay: `${driveDelayMs}ms` }}
        className="buggy-drive-enter absolute -right-4 top-1 h-[92px] w-[140px] opacity-90 transition-transform duration-500 group-hover:-translate-x-1"
      >
        <Image
          src="/buggy.webp"
          alt="Buggy EV"
      ...
      {/* Favorite star — hanya tampil untuk authenticated user */}
